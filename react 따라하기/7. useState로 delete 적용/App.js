...
function App() {
  ...
  const [deleteOb, setDeleteOb] = useState([]);
  ...
  if(mode == 'Welcome'){
    ...
    contentController = <>
      ...
      <li>
        <button onClick={()=>{
          const newTopics = [];//delete 된 문자열 넣을 변수 
          for(let i=0; i<topics.length; i++){
            if(topics[i].id != id){//현재 선택된 id를 빼고 모두 배열에 추가 
              newTopics.push(topics[i]);
            }else{
              setDeleteOb(topics[i]);
            }
          }
          setTopics(newTopics);
          //setMode('Welcome');
    ...
    content = <Delete title={deleteOb.title} body={deleteOb.body}>
