...
public class all extends Applet implements ActionListener {
	...
	public void TotalgetDBSearch() {
		...
		try {
			...
			int num = 1;
			ResultSet result = stmt.executeQuery("Select course.course_id,section.course_id FROM course left outer join section on course.course_id=section.course_id");//query문을 실행
			
			
			//1번째 = Select course.course_id,section.course_id FROM course left outer join section on course.course_id=section.course_id
			//2번째 = Select course.course_id,section.course_id FROM course right outer join section on course.course_id=section.course_id
			...
			while(result.next()) {//다음 정보가있을때까지 반복
				...
				String value = num +" "+ "course_id : " + col1 +" "+ "section_id : " + col2 + "\n";
